...
public class DiaLogic implements AutoCloseable{
    ...
    /**
     * Método que permite ingresar un regitro de Dia nuevo
     * @param info
     * @return 
     */
    public DiaEntity ingresaDia(DiaEntity info){
        DiaEntity infoRetorno=null;
        try{
            if(initOperation()){
                info.setId(maxId());
                sesion.save(info);
    ...
    /**
     * Métood que permite actualizar un registro de Dia existente
     * @param info
     * @return 
     */
    public DiaEntity actualizaDia(DiaEntity info){
        DiaEntity infoRetorno=null;
        try{
            if(initOperation()){
                sesion.update(info);
    ...
    /**
     * Método que trae toda la lista de registros de la Dia
     * @return 
     */
    public ArrayList<DiaEntity> listaDia(){
        ArrayList<DiaEntity>lista=new ArrayList<>();
        try{
            if(initOperation()){
                Criteria criteria=sesion.createCriteria(DiaEntity.class);
                lista=(ArrayList<DiaEntity>)criteria.list();
            }else{
                System.out.println("ERROR de validación al conectar");
            }
        }catch(Exception e){
    ...
    private Integer maxId(){
        ...
        try{
            if(initOperation()){
                Query query=sesion.createQuery("SELECT MAX(id) FROM DiaEntity");
