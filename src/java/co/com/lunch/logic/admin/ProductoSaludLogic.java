...
public class ProductoSaludLogic implements AutoCloseable {
    ...
    public ProductoSaludEntity actualizaProductoSalud(ProductoSaludEntity info) {
        ProductoSaludEntity infoRetorno = new ProductoSaludEntity();
        try {
            if (initOperation()) {
                info.setUltimaActualizacion(new Date(System.currentTimeMillis()));
                System.out.println("Fecha: " + info.getUltimaActualizacion());
                System.out.println("Prod: " + info.getProducto().getNombre());
                System.out.println("id: " + info.getIdProductosalud());
                sesion.update(info);
                //tx.commit();
                infoRetorno = info;
            } else {
            ...
            System.out.println("ERROR en el update del objeto: " + e);
        }
        return infoRetorno;
    }
    ...
    public ArrayList<ProductoSaludEntity> productosSNuevos(HashMap<Integer, Date> lista) {
        ArrayList<ProductoSaludEntity> retorna = new ArrayList<ProductoSaludEntity>();
        ArrayList<ProductoSaludEntity> listaTodos = listaProductoSalud();
        System.out.println("lista: " + lista.size());
        System.out.println("lista T: " + listaTodos.size());
        for (ProductoSaludEntity prod : listaTodos) {
            if (!lista.containsKey(prod.getIdProductosalud())) {
        ...
        for (ProductoSaludEntity prod : listaTodos) {
            if (lista.containsKey(prod.getIdProductosalud())) {
                if (prod.getUltimaActualizacion().after(lista.get(prod.getIdProductosalud()))) {
                    System.out.println("PROD: "+ prod.getIdProductosalud());
                    System.out.println("Fecha serv: "+prod.getUltimaActualizacion());
                    System.out.println("Fecha app: "+lista.get(prod.getIdProductosalud()));
    ...
    public void close() throws Exception {
        try {
            ...
            System.out.println("ERROR CLOSEABLE PSL: " + e.getMessage());
        }
    }
}
